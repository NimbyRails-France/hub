...
$exe=Get-Item "$PSScriptRoot/dist/NRFHub-0.2.0-Setup.exe"
$hash=(Get-FileHash -LiteralPath $exe.FullName).Hash.ToLowerInvariant()
@{schema=1;product='NRFHub';platform='windows-x64';version='0.2.0';url='https://github.com/NimbyRails-France/hub/releases/download/v0.2.0/NRFHub-0.2.0-Setup.exe';sha256=$hash;size=$exe.Length} | ConvertTo-Json | Set-Content "$PSScriptRoot/dist/hub-latest.json" -Encoding UTF8
