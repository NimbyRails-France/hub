...
Hub::Hub(){
 ...
 const auto config=readJson(dataDir_+"/settings.json");installed_=config["installed"].toObject();
 ...
 auto* options=new QHBoxLayout;automatic_=new QCheckBox("Mettre à jour automatiquement les projets installés");automatic_->setChecked(config["automatic"].toBool(true));options->addWidget(automatic_);options->addStretch();refresh_=new QPushButton("Actualiser le catalogue");options->addWidget(refresh_);layout->addLayout(options);
 connect(automatic_,&QCheckBox::toggled,this,[this]{save();});connect(refresh_,&QPushButton::clicked,this,[this]{refresh();});
 auto* selfUpdate=new Updater(this);selfUpdate->setEnabled(automatic_->isChecked());
 ...
 connect(updateButton,&QPushButton::clicked,this,[selfUpdate]{if(selfUpdate->ready())selfUpdate->restart();else selfUpdate->check();});connect(automatic_,&QCheckBox::toggled,this,[selfUpdate](bool value){selfUpdate->setEnabled(value);});
 ...
 status_=new QLabel("Chargement du catalogue…");layout->addWidget(status_);log_=new QPlainTextEdit;log_->setReadOnly(true);log_->setMaximumHeight(145);layout->addWidget(log_);
 ...
 detectGame();QTimer::singleShot(0,this,[this]{refresh();});auto* timer=new QTimer(this);connect(timer,&QTimer::timeout,this,[this]{refresh();});timer->start(15*60*1000);
}
void Hub::append(const QString& s){log_->appendPlainText(QTime::currentTime().toString("HH:mm:ss")+"  "+s);status_->setText(s);}
void Hub::closeEvent(QCloseEvent* event){if(operation_){append("Veuillez attendre la fin de l'installation avant de fermer le Hub.");event->ignore();return;}QMainWindow::closeEvent(event);}
...
void Hub::save(){if(!writeJson(dataDir_+"/settings.json",{{"gameDirectory",game_->text()},{"root",root_->text()},{"automatic",automatic_->isChecked()},{"installed",installed_}}))append("Impossible d'enregistrer les réglages");}
...
void Hub::render(){table_->setRowCount(projects_.size());for(int i=0;i<projects_.size();++i){auto p=projects_[i].toObject(),record=installed_[p["id"].toString()].toObject();QString reason;compatible(p,reason);QStringList values{p["name"].toString(),p["version"].toString(),record["version"].toString("—"),reason,record["directory"].toString()};for(int c=0;c<values.size();++c)table_->setItem(i,c,new QTableWidgetItem(values[c]));}}
void Hub::refresh(){if(busy_)return;setBusy(true);auto* reply=network_.get(request(QUrl(catalogue)));auto bytes=std::make_shared<QByteArray>();connect(reply,&QIODevice::readyRead,this,[reply,bytes]{*bytes+=reply->readAll();if(bytes->size()>1024*1024)reply->abort();});connect(reply,&QNetworkReply::finished,this,[this,reply,bytes]{*bytes+=reply->readAll();const bool ok=reply->error()==QNetworkReply::NoError&&bytes->size()<1024*1024;reply->deleteLater();setBusy(false);if(!ok){append("Catalogue indisponible · vos installations sont conservées");return;}auto doc=QJsonDocument::fromJson(*bytes).object();if(doc["schema"].toInt()!=1){append("Catalogue incompatible");return;}QJsonArray valid;QSet<QString> ids;for(auto v:doc["projects"].toArray()){auto p=v.toObject();if(!validProject(p)||ids.contains(p["id"].toString())){append("Catalogue rejeté : projet invalide");return;}ids.insert(p["id"].toString());valid.append(p);}projects_=valid;detectGame();render();append("Catalogue à jour · "+QString::number(projects_.size())+" projets publiés");if(automatic_->isChecked())checkUpdates();if(!busy_)updateSelf(doc["hub"].toObject());});}
...
void Hub::updateSelf(const QJsonObject& release){
 if(!automatic_->isChecked()||busy_||release.isEmpty()||QVersionNumber::fromString(release["version"].toString())<=QVersionNumber::fromString(HUB_VERSION))return;
 // Self-update uses the published installer. The user can review the release
 // while the installed-project updates remain fully automatic.
 append("Une nouvelle version du Hub est disponible : "+release["version"].toString());
}
