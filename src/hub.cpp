...
bool Hub::validProject(const QJsonObject& p){
 ...
 return id.match(p["id"].toString()).hasMatch()&&Releases::validVersion(p["version"].toString())&&hash.match(p["sha256"].toString()).hasMatch()&&
...
Hub::Hub(){
 ...
 const auto config=readJson(dataDir_+"/settings.json");installed_=config["installed"].toObject();notified_=config["notified"].toObject();channels_=config["channels"].toObject();releases_=new ReleaseClient(this);
 ...
 auto* options=new QHBoxLayout;automatic_=new QCheckBox("Mettre à jour automatiquement les projets installés");automatic_->setChecked(config["automatic"].toBool(true)&&!qApp->arguments().contains("--ui-test")&&!qApp->arguments().contains("--network-test"));options->addWidget(automatic_);options->addStretch();refresh_=new QPushButton("Actualiser les versions");options->addWidget(refresh_);layout->addLayout(options);
 connect(automatic_,&QCheckBox::toggled,this,[this]{save();});connect(refresh_,&QPushButton::clicked,this,[this]{refresh();});
 auto* selfUpdate=new Updater(this);selfUpdate_=selfUpdate;selfUpdate->setChannel(Releases::selectedChannel(channels_,"hub"));selfUpdate->setEnabled(automatic_->isChecked());
 auto* updateRow=new QHBoxLayout;auto* hubChannel=new QComboBox;hubChannel->setObjectName("hub-channel");hubChannel->addItem("Hub · Stable","stable");hubChannel->addItem("Hub · Bêta","beta");hubChannel->addItem("Hub · Alpha","alpha");hubChannel->setCurrentIndex(hubChannel->findData(Releases::selectedChannel(channels_,"hub")));updateRow->addWidget(hubChannel);connect(hubChannel,&QComboBox::currentIndexChanged,this,[this,hubChannel,selfUpdate]{channels_["hub"]=hubChannel->currentData().toString();selfNotified_=false;selfUpdate->setChannel(hubChannel->currentData().toString());save();selfUpdate->check(true);});auto* updateLabel=new QLabel;auto* updateButton=new QPushButton("Vérifier le Hub");updateRow->addWidget(updateLabel,1);updateRow->addWidget(updateButton);layout->addLayout(updateRow);
 connect(selfUpdate,&Updater::changed,this,[selfUpdate,updateLabel,updateButton]{updateLabel->setText(selfUpdate->status());updateButton->setText(selfUpdate->ready()?"Redémarrer le Hub pour appliquer":selfUpdate->canSwitch()?"Revenir à la version du canal":"Vérifier le Hub");});
 connect(updateButton,&QPushButton::clicked,this,[this,selfUpdate]{if(busy_){append("Veuillez attendre la fin des opérations.");return;}if(selfUpdate->ready())selfUpdate->restart();else if(selfUpdate->canSwitch()){if(QMessageBox::question(this,"Changer de version","Installer la version plus ancienne de ce canal ? Les mises à jour automatiques ne font jamais ce retour.")==QMessageBox::Yes)selfUpdate->check(true,true);}else selfUpdate->check(true);});connect(automatic_,&QCheckBox::toggled,this,[selfUpdate](bool value){selfUpdate->setEnabled(value);});
 table_=new QTableWidget(0,6);table_->setHorizontalHeaderLabels({"Projet","Canal","Disponible","Installé","Compatibilité","Dossier"});table_->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);table_->horizontalHeader()->setSectionResizeMode(5,QHeaderView::Stretch);table_->setSelectionBehavior(QAbstractItemView::SelectRows);table_->setSelectionMode(QAbstractItemView::SingleSelection);table_->setEditTriggers(QAbstractItemView::NoEditTriggers);layout->addWidget(table_,1);
 auto* actions=new QHBoxLayout;install_=new QPushButton("Installer / Mettre à jour");launch_=new QPushButton("Ouvrir");rollback_=new QPushButton("Revenir à la version précédente");remove_=new QPushButton("Désinstaller");for(auto* b:{install_,launch_,rollback_,remove_})actions->addWidget(b);auto* notes=new QPushButton("Version et changelog");actions->addWidget(notes);connect(notes,&QPushButton::clicked,this,[this]{const auto row=table_->currentRow();if(row<0)return;const auto url=projects_[row].toObject()["releaseUrl"].toString();if(!url.isEmpty())QDesktopServices::openUrl(QUrl(url));});layout->addLayout(actions);
 ...
 status_=new QLabel("Recherche des releases GitHub…");layout->addWidget(status_);log_=new QPlainTextEdit;log_->setReadOnly(true);log_->setMaximumBlockCount(500);log_->setMaximumHeight(145);layout->addWidget(log_);
 ...
 setupDesktop();restoreGeometry(QByteArray::fromBase64(config["geometry"].toString().toLatin1()));detectGame();if(!qApp->arguments().contains("--ui-test"))QTimer::singleShot(0,this,[this]{refresh();});auto* timer=new QTimer(this);connect(timer,&QTimer::timeout,this,[this]{refresh();});timer->start(15*60*1000);
}
...
}void Hub::setBusy(bool busy){busy_=busy;for(auto* b:{install_,remove_,rollback_,refresh_})b->setEnabled(!busy);game_->setEnabled(!busy);root_->setEnabled(!busy);for(auto* combo:findChildren<QComboBox*>())combo->setEnabled(!busy);}
void Hub::save(){if(!writeJson(dataDir_+"/settings.json",{{"gameDirectory",game_->text()},{"root",root_->text()},{"automatic",automatic_->isChecked()},{"installed",installed_},{"channels",channels_},{"notified",notified_},{"geometry",QString::fromLatin1(saveGeometry().toBase64())}}))append("Impossible d'enregistrer les réglages");}
...
void Hub::render(){
 ...
 table_->setRowCount(projects_.size());
 for(int i=0;i<projects_.size();++i){
  const auto p=projects_[i].toObject();const auto id=p["id"].toString();const auto record=installed_[id].toObject();QString reason;
  if(p["unavailable"].toBool())reason=p["error"].toString();else compatible(p,reason);
  const auto available=p["installedOnly"].toBool()?"Local (non publié)":p["unavailable"].toBool()?"Indisponible":p["version"].toString();
  QStringList values{p["name"].toString(),"",available,record["version"].toString("—"),reason,record["directory"].toString()};
  for(int c=0;c<values.size();++c)table_->setItem(i,c,new QTableWidgetItem(values[c]));
  table_->item(i,2)->setToolTip(p["changelog"].toString());
  auto* combo=new QComboBox;combo->setObjectName("channel-"+id);combo->addItem("Stable","stable");combo->addItem("Bêta","beta");combo->addItem("Alpha","alpha");
  combo->setCurrentIndex(combo->findData(Releases::selectedChannel(channels_,id)));combo->setEnabled(!busy_);table_->setCellWidget(i,1,combo);
  connect(combo,&QComboBox::currentIndexChanged,this,[this,id,combo]{channels_[id]=combo->currentData().toString();save();refresh();});
 }
}
void Hub::refresh(){
 if(busy_)return;setBusy(true);append("Recherche des releases GitHub…");
 releases_->discover([this](QStringList repositories,QString error){
  if(!error.isEmpty()){for(int i=0;i<projects_.size();++i){auto p=projects_[i].toObject();p["unavailable"]=true;p["error"]=error;projects_[i]=p;}setBusy(false);render();append(error+" · installations conservées");return;}
  repositories_=repositories;projects_={};refreshReleases(0);
 });
}

void Hub::checkUpdates(){if(busy_||!automatic_->isChecked())return;for(int i=0;i<projects_.size();++i){auto p=projects_[i].toObject(),record=installed_[p["id"].toString()].toObject();if(!p["unavailable"].toBool()&&!p["installedOnly"].toBool()&&!record.isEmpty()&&Releases::compare(p["version"].toString(),record["version"].toString())>0){QString reason;if(compatible(p,reason)){installProject(i,true);if(busy_)return;}else append(p["name"].toString()+" : mise à jour différée · "+reason);}}}
void Hub::installProject(int row,bool automatic){if(busy_||row<0||row>=projects_.size())return;auto p=projects_[row].toObject();if(p["unavailable"].toBool()){append(p["error"].toString());return;}if(p["installedOnly"].toBool()){append("Ce projet local doit être mis à jour avec son paquet local.");return;}QString reason;detectGame();if(!compatible(p,reason)){append(reason);return;}if(running(L"NIMBYRails.exe")||running(L"NimbyTco.exe")){append("Installation différée : fermez NIMBY Rails et le TCO.");return;}auto record=installed_[p["id"].toString()].toObject();QString destination=record["directory"].toString();if(destination.isEmpty()){if(automatic)return;auto parent=QFileDialog::getExistingDirectory(this,"Choisir le dossier parent du projet",root_->text());if(parent.isEmpty())return;destination=QDir(parent).absoluteFilePath(p["id"].toString());}if(!automatic&&QMessageBox::question(this,"Installation",p["name"].toString()+" "+p["version"].toString()+" ("+p["channel"].toString()+") sera installé dans :\n"+destination+"\n\nLe SDK installe aussi son chargeur dans le jeu. Continuer ?")!=QMessageBox::Yes)return;download(p,destination);}
